...
plot_kiva_gdp <- ggplot(kiva_gdp, aes(x = `GDP per Capita`, y = `Loan Sum`, country = `Country`)) +
  ...
  labs(x = "GDP per capita (USD)", y = "Sum of Loans (Million USD)", title = "The total sum of Kiva loans against the GDP per capita for each country")
...
```{r}
ggplotly(ggplot(agriculture_by_region, aes(x = region, y = total_loans_sum)) +
 geom_col(fill = "black", stat = "identity") +  
 ylab("Total Loans (Million USD)") +
  xlab("Regions") + ggtitle("Total Loans Distributed Per Region") +
...
ggplotly(ggplot(data = regions_data, aes(x = region, y = value, fill = gender)) + 
           ...
           geom_bar(stat = "identity", position = "dodge") + scale_fill_manual(values = c("male" = "black", "female" = "grey")) +
