...
To answer this question an interactive scatter plot was produced using the below code and GDP data from the tmap package. Hovering above a data point will bring up the country's name, its GDP per capita and its total loan sum. Both axis are logarithmic with a base of 10 in order to spread out the data so relationships can be drawn.

```{r, warning = FALSE, message = FALSE}
# Merging the world_gdp data and the kiva dataset to generate an interactive plot
kiva_sum <- kiva_loans %>%
  group_by(country) %>%
...
The plot has clustering towards the top left, implying that countries with a lower GDP per capita account for the majority of loaned money. There is however no direct linear correlation between the total loaned amount and the GDP per capita. Countries with the highest total loan sum tend to be developing countries with a low GDP per capita. Agriculture tends to be the largest sector in terms of these countries GDP and employment. (https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=7d15e1f8e20c97dd4eeeccd2ad3f5e3e4255b831#:~:text=Agricultural%20Spending,shares%20in%20GDP%20and%20employment.). This leads into IQ 2 which looks into how kiva funding is distributed per sector. 
